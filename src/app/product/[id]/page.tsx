    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '3rem 1.5rem 5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '3rem', marginBottom: '4rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {/* <p style={{ fontSize: '0.6rem', letterSpacing: '0.28em', textTransform: 'uppercase', color: 'var(--teal-light)', marginBottom: 6 }}>
            {product.category}
          </p> */}
          <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 700, color: 'var(--cream)', lineHeight: 1.1, marginBottom: '0.75rem' }}>
            {product.content}
          </h1>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid var(--border)' }}>
            {[
              ...
              ['⚖️', product.weight],
              ['✅', 'FSSAI Certified'],
              ...(product.shelfLife ? [['⏱️', `Shelf Life: ${product.shelfLife}`]] : []),
            ].map(([icon, label]) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.5rem', marginBottom: '4rem' }}>
        ...
        <div style={{ background: 'var(--dark3)', border: '1px solid var(--border)', borderLeft: '3px solid var(--gold)', borderRadius: 10, padding: '2rem' }}>
          <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '0.72rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '1rem' }}>
            About This Product
          </h2>
        ...
        <div style={{ background: 'var(--dark3)', border: '1px solid var(--border)', borderLeft: '3px solid var(--teal-light)', borderRadius: 10, padding: '2rem' }}>
          <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '0.72rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--teal-light)', marginBottom: '1rem' }}>
            Key Benefits
          </h2>
          <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {product.benefits.map(b => (
              <li key={b} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: '0.82rem', color: '#8a7a68', lineHeight: 1.6 }}>
                <span style={{ color: 'var(--teal-light)', flexShrink: 0, marginTop: 2 }}>✓</span>{b}
              </li>
            ))}
          </ul>
        </div>
        ...
        {(product.shelfLife || (product.storageConditions && product.storageConditions.length > 0)) && (
          <div style={{ background: 'var(--dark3)', border: '1px solid rgba(91,163,208,0.2)', borderLeft: '3px solid #5ba3d0', borderRadius: 10, padding: '2rem' }}>
            <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '0.72rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#5ba3d0', marginBottom: '1rem' }}>
              Shelf Life &amp; Storage
            </h2>
            {product.shelfLife && (
              <div style={{ marginBottom: '1.1rem' }}>
                <p style={{ fontSize: '0.62rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#7a6e60', marginBottom: 4 }}>Shelf Life</p>
                <p style={{ fontFamily: 'Cinzel, serif', fontSize: '1.1rem', color: '#5ba3d0', fontWeight: 600 }}>{product.shelfLife}</p>
              </div>
            )}
            {product.storageConditions && product.storageConditions.length > 0 && (
              <div>
                <p style={{ fontSize: '0.62rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#7a6e60', marginBottom: 8 }}>Storage Conditions</p>
                <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 9 }}>
                  {product.storageConditions.map(cond => (
                    <li key={cond} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: '0.82rem', color: '#8a7a68', lineHeight: 1.6 }}>
